...
class UserAPITest(APITestCase):
    ...
    def test_user_update_with_invalid_field_failure(self):
        invalid_data = {
            "username": "updateeeeeeeeeeeeee!", # 10자로 제한, 특수문자 불가
            "email": "update@update.com",
            "password": "test123!"
        }
        response = self.client.put(f"/api/users/{self.existing_user.id}/", invalid_data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
